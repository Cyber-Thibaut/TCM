...
async function fetchLineDetails() {
  try {
    ...
    return line || {
      id: 901,
      nom: "Bellevue <> Haute-Ville",
      type: "Volca'Demande",
      description: "Service de transport à la demande desservant les secteurs de Bellevue et Haute-Ville.",
      stats: {
        nombre_arrets: 10,
        temps_trajet: "12 minutes",
        longueur_ligne: "4 km"
      }
    };
  } catch (error) {
...
function renderLineDetails(lineData) {
  ...
  plan.onerror = function() {
    ...
    fallback.className = "bg-base-200 dark:bg-base-300 rounded-xl p-8 text-center";
    fallback.innerHTML = `
  ...
  type.className = "badge badge-lg badge-outline badge-warning px-4 py-3 text-base font-semibold";
  ...
  distanceContainer.className = "flex flex-col items-center justify-center p-6 bg-base-200 dark:bg-base-300 rounded-xl shadow-lg";
  ...
  distanceTitle.className = "text-sm font-medium text-base-content/70 uppercase tracking-wider";
  ...
  tadInfoContainer.className = "bg-orange-50 dark:bg-orange-900/20 rounded-xl p-6 mb-6";
  ...
  if (lineData.description) {
    ...
    description.className = "text-lg text-base-content/80 leading-relaxed text-justify mb-6";
...
async function updateBusTimes() {
  ...
  if (nextBusTime) {
    nextBusTime.innerHTML = `
      <div class="card bg-gradient-to-r from-blue-500 to-purple-600 text-base-content shadow-xl">
...
function updateCurrentTime() {
  ...
  if (destination) {
    destination.innerHTML = `<h2 class="text-4xl text-primary-content dark:text-primary font-bold">Volca'Demande 901 ♿</h2><p class="text-lg text-primary-content/80 dark:text-primary/80 mt-2">Bellevue ↔ Haute-Ville</p>`;
  }
}
