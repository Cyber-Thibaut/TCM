...
function renderTrafficInfo(trafficData) {
  ...
  if (trafficData && trafficData.lignes) {
    ...
    // Filtrer les lignes 602
    const lignes602 = trafficData.lignes.filter(ligne => ligne.ligne === "602");
    
    lignes602.forEach(ligne => {
    ...
    if (infosActivesCount > 0) {
      ...
      trafficCard.innerHTML = `
        <div role="alert" class="alert alert-error shadow-lg">
          ...
          <div>
            <h3 class="font-bold">Perturbation sur la ligne 602</h3>
            <div class="text-xs">${incidentText}</div>
          </div>
          <div>
            <a href="details.html#602" class="btn btn-sm btn-error">
              Plus d'infos
            </a>
          </div>
        </div>
      ...
      trafficCard.innerHTML = `
        <div role="alert" class="alert alert-success shadow-lg">
          ...
          <div>
            <h3 class="font-bold">Trafic normal sur la ligne 602</h3>
...
function renderLineDetails(ligne) {
  ...
  if (destinationElement) {
    destinationElement.textContent = ligne.destination || "Ligne 602";
  }
}
...
async function updateBusTimes() {
  ...
  if (now.getHours() >= 6 && now.getHours() < 9) {
    ...
    <div class="p-6 w-3/4 lg:w-1/2 bg-gradient-to-r from-primary to-secondary rounded-xl shadow-lg text-center">
      <h2 class="text-5xl font-semibold text-base-content">🚸 Direction Lycée Jeanne d'Arc</h2>
    </div>
  </div>`;
  } else if (now.getHours() >= 11 && now.getHours() < 19) {
    destinationElement.innerHTML = `<div class="w-full flex justify-center py-8">
    <div class="p-6 w-3/4 lg:w-1/2 bg-gradient-to-r from-primary to-secondary rounded-xl shadow-lg text-center">
      <h2 class="text-5xl font-semibold text-base-content">🏠 Retour vers Lezoux Hôtel de Ville</h2>
    </div>
  </div>`;
  } else {
    destinationElement.innerHTML = "";
...
async function init() {
  ...
  if (lineDetailsData) {
    ...
    // Créer une ligne par défaut pour 602
    const defaultLine = {
      id: 602,
      nom: "Ligne 602",
  ...
  // Charger et afficher les informations de trafic spécifiques à la ligne 602
