...
function calculateFollowingBusTime(weekday, hour, minute, frequency) {
    ...
    if (isVacation === true) {
        if (weekday === 6) { // Samedi
            ...
            // Fréquences pour Samedi
            if (hour >= 4 && hour < 7) {
                followingFrequency = -1; // Creuse
            } else if (hour >= 7 && hour < 9) {
                followingFrequency = 20; // Pointe
            } else if (hour >= 9 && hour < 16) {
                followingFrequency = -1; // Creuse
            } else if (hour >= 16 && hour < 20) {
                followingFrequency = 20; // Pointe
...
function updateCurrentTime() {
    ...
    document.getElementById('destination').innerHTML = `<h2 class="text-4xl text-primary-content dark:text-primary font-bold">Direction Lycée Karl Marx ♿</h2><br>`;
