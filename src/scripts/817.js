...
function calculateFollowingBusTime(weekday, hour, minute, frequency) {
    ...
    if (estFerie) {
        ...
        // Fréquences pour Samedi
        if (hour >= 4 && hour < 7) {
            followingFrequency = 4; // Creuse
        } else if (hour >= 7 && hour < 9) {
            followingFrequency = 4; // Pointe
        } else if (hour >= 9 && hour < 16) {
            followingFrequency = 4; // Creuse
        } else if (hour >= 16 && hour < 20) {
            followingFrequency = 4; // Pointe
...
function updateBusTimes() {
    ...
    if (nextBusFreq === -1) {
        if ((hour === 0 && minute >= 0) || (hour >= 1 && hour < 4)) {
            ...
            nextBus = `<div role="alert" class="alert alert-info"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="stroke-current shrink-0 w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><span>Aucune épreuve d'equi handi en cours. Reportez vous au calendrier officiel pour connaitre la prochaine date d'épreuve.</span></div>`;
        }
...
function updateCurrentTime() {
    ...
    document.getElementById('destination').innerHTML = `<h2 class="special-font text-4xl text-primary-content dark:text-primary font-bold">Direction la Maison des Sport ♿</h2><br>`;
